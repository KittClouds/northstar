use super::*;
use crate::data_plane::providers::ons::{binding_for_code, ONS_SERIES};

impl MacroPlane {
    pub fn ingest_complete_ons_refresh(
        &mut self,
        receipts: Vec<FetchedOnsReceipt>,
        next_refresh_ns: i64,
    ) -> Result<MacroIngestReport, MacroPlaneError> {
        validate_complete_ons_refresh(&receipts)?;
        ...
        for fetched in receipts {
            ...
            let receipt = RawReceipt {
                ...
                metadata: format!("GET /generator binding={}", fetched.provider_code).into_bytes(),
            ...
            let stats = self.ons_decoder.decode(
                RawReceiptRef {
                    id: receipt.id,
        ...
        let snapshot = self.published_snapshot(
            ...
            if committed_events == 0 {
                "ONS UK macro current / no new observations"
            } else {
                "ONS UK macro committed to canonical replay"
            },
            next_refresh_ns,
        );
...
fn validate_complete_ons_refresh(receipts: &[FetchedOnsReceipt]) -> Result<(), MacroPlaneError> {
    if receipts.len() != ONS_SERIES.len() {
        return Err(MacroPlaneError::IncompleteOnsRefresh {
            expected: ONS_SERIES.len(),
            actual: receipts.len(),
        });
    }
    let mut seen = HashSet::with_capacity(ONS_SERIES.len());
    for receipt in receipts {
        if binding_for_code(receipt.provider_code).is_none() {
            return Err(MacroPlaneError::UnknownOnsSeries(
