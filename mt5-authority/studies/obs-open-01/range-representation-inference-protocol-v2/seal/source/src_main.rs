use obs_open_03bp2::seal::{build, copy_seal, finalize, verify};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.get(1).map(String::as_str){
        ...
        _=>return Err("usage: obs-open-03bp2 <build REPO OUT|finalize LEFT RIGHT|copy-seal BUILD SEAL|verify ROOT>".into())
    }
    Ok(())
}
