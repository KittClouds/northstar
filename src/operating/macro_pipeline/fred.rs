use super::*;
use crate::data_plane::providers::fred::{binding_for_code, FRED_SERIES};

impl MacroPlane {
    pub fn ingest_complete_fred_refresh(
        &mut self,
        receipts: Vec<FetchedFredReceipt>,
        next_refresh_ns: i64,
    ) -> Result<MacroIngestReport, MacroPlaneError> {
        validate_complete_fred_refresh(&receipts)?;
        ...
        for fetched in receipts {
            ...
            let stats = self.fred_decoder.decode(
                RawReceiptRef {
                    id: receipt.id,
        ...
        let snapshot = self.published_snapshot(
            ...
            if committed_events == 0 {
                "FRED rates current / no new observations"
            } else {
                "FRED rates committed to canonical replay"
            },
            next_refresh_ns,
        );
...
fn validate_complete_fred_refresh(receipts: &[FetchedFredReceipt]) -> Result<(), MacroPlaneError> {
    if receipts.len() != FRED_SERIES.len() {
        return Err(MacroPlaneError::IncompleteFredRefresh {
            expected: FRED_SERIES.len(),
            actual: receipts.len(),
        });
    }
    let mut seen = HashSet::with_capacity(FRED_SERIES.len());
    for receipt in receipts {
        if binding_for_code(receipt.provider_code).is_none() {
            return Err(MacroPlaneError::UnknownFredSeries(
