use super::*;
use crate::data_plane::providers::ecb::{binding_for_code, ECB_SERIES};

impl MacroPlane {
    pub fn ingest_complete_ecb_refresh(
        &mut self,
        receipts: Vec<FetchedEcbReceipt>,
        next_refresh_ns: i64,
    ) -> Result<MacroIngestReport, MacroPlaneError> {
        validate_complete_ecb_refresh(&receipts)?;
        ...
        for fetched in receipts {
            ...
            let receipt = RawReceipt {
                ...
                metadata: format!("GET /service/data/FM binding={}", fetched.provider_code)
                    .into_bytes(),
            ...
            let stats = self.ecb_decoder.decode(
                RawReceiptRef {
                    id: receipt.id,
        ...
        let snapshot = self.published_snapshot(
            ...
            if committed_events == 0 {
                "ECB policy rates current / no new observations"
            } else {
                "ECB policy rates committed to canonical replay"
            },
            next_refresh_ns,
        );
...
fn validate_complete_ecb_refresh(receipts: &[FetchedEcbReceipt]) -> Result<(), MacroPlaneError> {
    if receipts.len() != ECB_SERIES.len() {
        return Err(MacroPlaneError::IncompleteEcbRefresh {
            expected: ECB_SERIES.len(),
            actual: receipts.len(),
        });
    }
    let mut seen = HashSet::with_capacity(ECB_SERIES.len());
    for receipt in receipts {
        if binding_for_code(receipt.provider_code).is_none() {
            return Err(MacroPlaneError::UnknownEcbSeries(
