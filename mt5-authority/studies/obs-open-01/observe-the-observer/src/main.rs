...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.get(1).map(String::as_str) {
        ...
        _ => {
            return Err(
                "usage: obs-open-observe-the-observer <build REPO OUT|verify SEAL|finalize A B SEAL|topology-build REPO OUT|topology-verify SEAL|topology-finalize A B SEAL|rt-build REPO OUT|rt-verify SEAL|rt-finalize A B SEAL|intervention-build REPO OUT|intervention-verify SEAL|intervention-finalize A B SEAL|covariation-build REPO OUT|covariation-verify SEAL|covariation-finalize A B SEAL|authority-impact-build REPO OUT|authority-impact-verify SEAL|authority-impact-finalize A B SEAL|authority-surface-build REPO OUT|authority-surface-verify SEAL|authority-surface-finalize A B SEAL|literature-build REPO OUT|literature-verify SEAL|literature-finalize A B SEAL|sol-build REPO OUT|sol-verify SEAL|sol-finalize A B SEAL|kammi-build REPO OUT|kammi-verify SEAL|kammi-finalize A B SEAL>"
                    .into(),
            );
        }
    }
