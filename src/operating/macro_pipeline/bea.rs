use super::*;
use crate::data_plane::providers::bea::{binding_for_code, BEA_SERIES};

impl MacroPlane {
    pub fn ingest_complete_bea_refresh(
        &mut self,
        receipts: Vec<FetchedBeaReceipt>,
        next_refresh_ns: i64,
    ) -> Result<MacroIngestReport, MacroPlaneError> {
        validate_complete_bea_refresh(&receipts)?;
        ...
        for fetched in receipts {
            ...
            let receipt = RawReceipt {
                ...
                metadata: format!("GET /api/data binding={}", fetched.provider_code).into_bytes(),
            ...
            let stats = self.bea_decoder.decode(
                RawReceiptRef {
                    id: receipt.id,
        ...
        let snapshot = self.published_snapshot(
            ...
            if committed_events == 0 {
                "BEA GDP/PCE current / no new observations"
            } else {
                "BEA GDP/PCE committed to canonical replay"
            },
            next_refresh_ns,
        );
...
fn validate_complete_bea_refresh(receipts: &[FetchedBeaReceipt]) -> Result<(), MacroPlaneError> {
    if receipts.len() != BEA_SERIES.len() {
        return Err(MacroPlaneError::IncompleteBeaRefresh {
            expected: BEA_SERIES.len(),
            actual: receipts.len(),
        });
    }
    let mut seen = HashSet::with_capacity(BEA_SERIES.len());
    for receipt in receipts {
        if binding_for_code(receipt.provider_code).is_none() {
            return Err(MacroPlaneError::UnknownBeaSeries(
