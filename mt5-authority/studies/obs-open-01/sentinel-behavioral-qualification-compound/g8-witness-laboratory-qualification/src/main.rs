use obs_open_04a_g8::seal::{build, copy_seal, finalize, verify};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.get(1).map(String::as_str) {
        ...
        _ => return Err("usage: obs-open-04a-g8 <build REPO OUT|finalize LEFT RIGHT|copy-seal BUILD SEAL|verify ROOT>".into()),
    }
    Ok(())
}
