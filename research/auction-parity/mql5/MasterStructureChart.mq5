//+------------------------------------------------------------------+
//| MasterStructureChart.mq5                                         |
//| Independent chart projection over isolated structural producers. |
//+------------------------------------------------------------------+
...
{
   // A live chart can call the initial controller refresh from OnInit before
   // MT5 has sized the indicator buffers.  The Strategy Tester normally sizes
   // them earlier, which hid this boundary.  Rendering must not depend on the
   // optional projection buffers being available yet.
   if(ArraySize(NearestNodeBuffer) < 1 ||
      ArraySize(LowerNodeBuffer) < 1 ||
      ArraySize(UpperNodeBuffer) < 1 ||
      ArraySize(NodeCountBuffer) < 1)
      return;
...
{
   if(!g_is_tester)
      RefreshMaster(false, false, false);
}
                ...
                const int &spread[])
{
   ...
   bool allow_cached_structure = g_is_tester && InpTesterCacheUnchangedStructure;
   RefreshMaster(false, false, allow_cached_structure);
   return rates_total;
}
